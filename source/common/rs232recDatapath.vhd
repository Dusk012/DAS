...
entity rs232recController is
  generic (
    ...
    BAUDRATE : natural   -- velocidad de comunicación
  );
  port (
    clk          : in  std_logic;
    rst          : in  std_logic;
...
architecture Behavioral of rs232recController is
  ...
  constant BAUD_CNT_WIDTH  : natural := 16;   -- ancho suficiente
...
begin
 ...
 resetSynchronizer : synchronizer
  ...
  -- Contador de baudios: genera readRxD a mitad del período de bit
  baudCnt : process(clk)
  begin
  ...
  readRxD_int <= '1' when (baudCntCE = '1' and baud_counter = HALF_CYCLE - 1) else '0';
  readRxD     <= readRxD_int;
  ...
  begin
    if rising_edge(clk) then
      if rst = '1' then
        rx_shift <= (others => '0');
      elsif readRxD_int = '1' then
        rx_shift <= RxDSync & rx_shift(9 downto 1);
      end if;
    end if;
  end process;
   -- Los datos se toman de las posiciones 8..1 del registro (después del stop)
    ...
    if rising_edge(clk) then
      if rst = '1' then
        ...
        if bit_counter = 10 then
          bit_counter <= (others => '0');   -- vuelve a 0 (opcional)
        else
